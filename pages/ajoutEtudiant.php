<?php 
    ...
    if(isset($valider)){
        ...
        if(empty($nom)||empty($prenom)||empty($genre)||empty($date)||empty($telephone)||empty($adresse)||empty($promotion)||empty($licence)||empty($annee)){
            $errors="Tous les champs sont obligatoires";
        }else{
            if(telExiste($telephone)){
                $errors = "Ce numéro de téléphone existe déjà";  
            }else{
                $lastName = strtoupper(securite($nom));
                $firsrtName = ucfirst(securite($prenom));
                $telephone = ucfirst(securite($telephone));
                $date = securite($date);
                $adresse = securite($adresse);
                $matricule = generateMatricule($lastName, $firsrtName);
                $password = genererPassWord();
                ajoutUtilisateur($lastName, $firsrtName, $telephone, $matricule, $password, "User", "user.jpg");
                ajoutEtudiant($matricule, $lastName, $firsrtName, $genre, $date, $telephone, $adresse, $promotion, $licence, $annee);
                ajoutInscription($matricule,$annee, $licence, $date);
                    ...
                    <div class="card-body">
                        ...
                        <form class="new-added-form" method="POST" action="">
                            ...
                            <div class="row">
                                ...
                                <div class="col-xl-3 col-lg-6 col-12 form-group">
                                    <label for="fannee">Année Scolaire *</label>
                                    <select class="select2" id="fannee" name="annee">
